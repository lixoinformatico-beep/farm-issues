...
export default function CreateProblemaSheet({ open, onOpenChange, onCreated }) {
  const [form, setForm] = useState({
    ...
    estado: "Aberto", data_prevista: "", atribuido_a: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!payload.atribuido_a) delete payload.atribuido_a;
      const { data } = await api.post("/problemas", payload);
      toast.success("Problema criado com sucesso");
      onCreated?.(data);
      onOpenChange(false);
      setForm({ farmacia: "", laboratorio: "", consultor: "",
                descricao: "", tipologia: "TFO", prioridade: "Media",
                estado: "Aberto", data_prevista: "", atribuido_a: "" });
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4" data-testid="create-problema-form">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Atribuído a">
              <input data-testid="form-atribuido" className={inputCls}
                value={form.atribuido_a} onChange={(e) => update("atribuido_a", e.target.value)}
